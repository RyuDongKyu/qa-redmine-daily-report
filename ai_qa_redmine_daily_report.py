...
def get_yesterday_issues():
    ...
    # [중요] GitHub 서버(UTC) 시간을 한국 시간(KST)으로 변환
    ...
    for row in rows:
        try:
            ...
            if target_dash in input_time or target_dot in input_time:
                ...
                col_43_val = row[42].strip() if len(row) > 42 else ""
                
                # [수정됨] 42열이나 43열에 값이 '존재하면' 제외(Skip)
                if col_42_val or col_43_val:
                    continue
...
def generate_manual_report(date_str, issues, error_msg=""):
    ...
    <body style="font-family: Arial, sans-serif; color: #333;">
        <h2>안녕하세요, {date_str} 고객 이슈 리포트입니다.</h2>
...
def ask_gemini(date_str, issues):
    ...
    [작성 원칙 v9.5 - 필수 준수 사항]
    1. 인사말: "안녕하세요, {date_str} 고객 이슈 리포트입니다."로 시작할 것.
...
if __name__ == "__main__":
    ...
    if issues:
        ...
        send_email(f"[일일보고] {y_date} 고객 이슈 현황", final_html)
    else:
        send_email(f"[일일보고] {y_date} 고객 이슈 없음", f"<h3>{y_date} 등록된 이슈가 없습니다.</h3>")
        except: continue
